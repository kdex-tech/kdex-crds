...
type DynamicClientRegistration struct {
	...
	// at registration. Two values are named POLICIES that constrain the URL's
	// structure; every other value is matched as a literal URI scheme:
	//   - "http-loopback": http, but only to a loopback host — http://127.0.0.1,
	//     http://[::1], or http://localhost (native-app loopback, RFC 8252 §7.3).
	//   - "private-use": any non-http(s) reverse-DNS/dotted scheme, e.g.
	//     com.example.app:// (RFC 8252 §7.1; a bare single-label scheme is
	//     rejected under this policy).
	//   - any other value is a literal scheme allow: "https" permits https://…,
	//     "cursor" permits cursor://… (Cursor's native callback), "vscode"
	//     permits vscode://…, and so on — no per-scheme support code needed.
	// Regardless of the list, a bare "http" literal is never honored (use
	// "http-loopback") and dangerous schemes (javascript/data/file/vbscript/blob)
	// are always rejected. When a registration carries several redirect_uris,
	// only those whose scheme is allowed are kept (the rest are dropped, per
	// RFC 7591 §3.2.1); the request fails only when none is allowed. All
	// dynamically-registered clients are public + PKCE with an exact-matched
	// redirect_uri at /authorize.
	// +kubebuilder:validation:Optional
...
type Backend struct {
	...
	IngressPath string `json:"ingressPath,omitempty" protobuf:"bytes,1,opt,name=ingressPath"`

	Runtime `json:",inline" protobuf:"bytes,2,req,name=runtime"`

	// serverImage is the name of Backend image.
	// More info: https://kubernetes.io/docs/concepts/containers/images
	// +kubebuilder:validation:Optional
	// +kubebuilder:validation:MinLength=5
	ServerImage string `json:"serverImage,omitempty" protobuf:"bytes,3,opt,name=serverImage"`
	...
	// +kubebuilder:validation:Optional
	ServerImagePullPolicy corev1.PullPolicy `json:"serverImagePullPolicy,omitempty" protobuf:"bytes,4,opt,name=serverImagePullPolicy,casttype=PullPolicy"`
	...
	// +kubebuilder:validation:Optional
	// +kubebuilder:validation:MinLength=5
	StaticImage string `json:"staticImage,omitempty" protobuf:"bytes,5,opt,name=staticImage"`
	...
	// Defaults to Always if :latest tag is specified, or IfNotPresent otherwise.
	// +kubebuilder:validation:Optional
	StaticImagePullPolicy corev1.PullPolicy `json:"staticImagePullPolicy,omitempty" protobuf:"bytes,6,opt,name=staticImagePullPolicy,casttype=PullPolicy"`
}
...
type Builder struct {
	...
	// cache configures the kpack build cache PVC sized per Builder.
	// Forwarded onto the kpack.io/Image's spec.cache.volume.size.
	// kpack's own default when this field is unset on the Image is "2G",
	// which is too small for typical Go/Rust dep trees; host-manager
	// falls back to "10Gi" when this field is nil so that the default
	// is reasonable without per-function tuning. The PVC's StorageClass
	// is the cluster default. Once a PVC is provisioned at a given size,
	// k8s forbids shrinking it (the kpack controller will error out
	// every reconcile until the field is bumped back up or the PVC is
	// deleted); bump this value up freely, never down.
	// +kubebuilder:validation:Optional
	Cache *BuildCache `json:"cache,omitempty" protobuf:"bytes,9,opt,name=cache"`
}

// BuildCache configures the kpack build cache PVC for a Builder.
type BuildCache struct {
	// volumeSize is the requested PVC size for the kpack build cache.
	// Forwarded onto the kpack.io/Image's spec.cache.volume.size.
	// +kubebuilder:validation:Required
	// +kubebuilder:default="10Gi"
	VolumeSize resource.Quantity `json:"volumeSize" protobuf:"bytes,1,req,name=volumeSize"`
}
