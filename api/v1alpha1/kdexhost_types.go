...
// +kubebuilder:validation:XValidation:rule=`self.devMode == true || (has(self.auth) && has(self.auth.jwt) && has(self.auth.jwt.jwtKeysSecrets) && self.auth.jwt.jwtKeysSecrets.size() > 0)`,message="'spec.devMode' must be true or 'spec.auth.jwt.jwtKeysSecrets' must be specified"
